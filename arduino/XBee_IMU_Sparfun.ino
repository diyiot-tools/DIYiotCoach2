...
{ 
  ...
  sendString = "@";  
  //char(10) - new line : only for testing 
  sendString = sendString + printAccel() + "*" + printGyro();


  
  calcuMag();
  sendString = sendString + '*' + printHeading_2(mx, my, mz, R_y, R_x) + '*' + R_y + '*' + R_x + '#' + char(10); 
  
  //XBee.write(Serial.print("test"));
  char test[60];
  sendString.toCharArray(test,60);
...
String printHeading_2(float hx, float hy, float hz, float rol, float pitc)
{
  float heading;
